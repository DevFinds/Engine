0. Смонтируйте iso-файл R2023b_Windows.iso на виртуальный диск.
     Для Windows 8 и ниже вам, вероятно, понадобится программа Daemon Tools Lite (или аналогичная).

1. Запустите setup.exe с виртуального диска, и если вы увидите форму ввода логина/пароля/подписки (вы дали доступ к интернету для программы установки)
     то в правом верхнем углу в разделе "Дополнительные параметры" выберите режим установки "У меня есть ключ установки файла".
     Если доступ к интернету отсутствует, то нужный режим установки будет выбран автоматически, и вам не нужно будет выбирать его вручную

2. Когда появится запрос "Введите ключ установки файла", введите
...
2.1 Параллельный сервер MATLAB R2023a:
...
3. Когда вам будет предложено "Выбрать файл лицензии", выберите файл "license.lic" из папки с файлом Matlab913_R2022b_Win64.iso.

4. Затем выберите папку, в которую вы хотите установить Matlab (<matlabfolder>).

Переведено с помощью DeepL.com (бесплатная версия)

5. Когда вам будет предложено "Выбрать продукты", выберите необходимые компоненты.
     Если выбрать все компоненты, то Matlab потребуется около 30 Гб дискового пространства и несколько большее время запуска.
     Если выбрать только "MATLAB", то Matlab потребуется около 3 Гб дискового пространства.
     Для лучшего времени запуска Matlab лучше установить на SSD-диск, так что, скорее всего, вы не хотите тратить место на SSD-диске понапрасну.

6. Затем в "Выбрать параметры" выберите "Добавить ярлык на рабочий стол".

7. Прогресс установки компонентов может отображаться некорректно (например, всегда показывать 0%)... просто подождите
     Или, если процесс установки занимает слишком много времени, начните следить за размером папки <matlabfolder>.
     Если через несколько минут размер не увеличивается, перезапустите установку с шага 1.

8. После завершения установки скопируйте файл "libmwlmgrimpl.dll" из папки с файлом R2023b_Windows.iso
     в уже существующую папку "<matlabfolder>\bin\win64\matlab_startup_plugins\lmgrimpl"
     С ПЕРЕЗАПИСЬЮ СУЩЕСТВУЮЩЕГО ФАЙЛА (<matlabfolder> - это место, куда вы выбрали установку Matlab на шаге 4)
     Если вас НЕ спросили о перезаписи, значит вы что-то делаете не так (или Matlab не был успешно установлен)!!!
8.1 для polyspace замените libmwlmgrimpl по следующему пути:C:\Program Files\Polyspace\R2023a\bin\win64\matlab_startup_plugins\lmgrimpl

8.1 для polyspace замените libmwlmgrimpl по следующему пути:C:\Program Files\Polyspace\R2023a\bin\win64\matlab_startup_plugins\lmgrimpl

9. Если ярлык на рабочем столе не был создан (или был создан плохой ярлык)
     , создайте новый ярлык или измените существующий, чтобы он запускал
     "<matlabfolder>\bin\win64\MATLAB.exe".

10. Работайте с Matlab :)

P.S Некоторые аддоны или продукты могут быть недоступны!

P.S.
Если установка зависнет на шаге 1-7, то закройте ее и запустите установку снова с шага 1.

P.S.2
При обновлении/изменении уже работающего Matlab нет необходимости выполнять шаг 3
Шаг 8 может потребоваться повторить (если при обновлении/замене Matlab был перезаписан файл "libmwlmgrimpl.dll")
Если после обновления/замены вы получаете ошибку при запуске Matlab, то сначала попробуйте повторить шаг 8
